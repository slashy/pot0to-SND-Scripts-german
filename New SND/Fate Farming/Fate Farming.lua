...
configs:
  ...
  Chocobo Haltung:
    default: "Heilen"
    description: Options - Mitkommen/Freie Haltung/Verteidigen/Heilen/Angreifen/Keine. Will not summon chocobo if set to "Keine"
    type: string
  Gizar-Blatt kaufen?:
    default: true
    description: Automatically buys a 99 stack of Gysahl Greens from the Limsa gil vendor if none in inventory
    type: boolean
  Ignoriere FATE wenn Fortschritt über (%):
    default: 80
    type: int
    min: 0
    max: 100
    required: true
  Ignoriere FATE wenn Restzeit wengier als (minuten):
    default: 3
    type: int
    min: 0
    max: 100
  ...
  Ignoriere FATE wenn Restzeit wengier als (minuten):
    ...
    max: 100
    required: true
  Ignoriere Boss-FATE bis Fortschritt mindestens (%):
    default: 0
    type: int
    min: 0
    max: 100
    required: true
  Ignoriere Special-FATE bis Fortschritt mindestens (%):
    default: 20
    type: int
    min: 0
    max: 100
  ...
  Sammel-FATEs machen?:
    default: true
    type: boolean
  Nur Bonus-FATEs machen?:
    default: false
    type: boolean
  Maximale Nahkämpfer-Reichweite:
    default: 2.5
    type: float
    min: 0
    max: 30
    required: true
  Maximale Fernkämpfer-Reichweite:
    default: 20
    type: float
    min: 0
    max: 30
  ...
  Forlorns:
    ...
    required: true
  Instanz Wechseln, wenn keine FATESs?:
    default: true
    type: boolean
  Zweifarbige Edelsteine eintauschen gegen:
    default: Goldenes Handelszertifikat für zweifarbige Edelsteine
    type: string
...
function TeleportTo(aetheryteName)
    ...
    yieldd\("/wait 1") -- wait for casting to begin
    while Svc.Condition[CharacterCondition.casting] do
        Dalamud.Log("[FATE] Casting teleport...")
        yieldd\("/wait 1")
    end
    yieldd\("/wait 1") -- wait for that microsecond in between the cast finishing and the transition beginning
...
function ChangeInstance()
    ...
    yieldd\("/wait 1") -- wait for instance transfer to register
...
function MountState()
    if Svc.Condition[CharacterCondition.mounted] then
        yieldd\("/wait 1") -- wait a second to make sure you're firmly on the mount
...
function MoveToFate()
    ...
    if GetDistanceToPoint(CurrentFate.position) < 60 then
        if Svc.Targets.Target ~= nil then
            ...
            yieldd\("/wait 0.5") -- give it a moment to make sure the target sticks
            return
        end
    end
...
function InteractWithFateNpc()
    if InActiveFate() or CurrentFate.startTime > 0 then
        ...
        State = CharacterState.doFate
        Dalamud.Log("[FATE] State Change: DoFate")
        yieldd\("/wait 1") -- give the fate a second to register before dofate and lsync
...
function DoFate()
    ...
    elseif InActiveFate() and (CurrentFate.fateObject.MaxLevel < Player.Job.Level) and not Player.IsLevelSynced then
        Engines.Run("/lsync")
        yieldd\("/wait 0.5") -- give it a second to register
    ...
    elseif CurrentFate.isCollectionsFate then
        yieldd\("/wait 1") -- needs a moment after start of fate for GetFateEventItem to populate
    ...
    if not Svc.Condition[CharacterCondition.inCombat] then
        if Svc.Targets.Target ~= nil then
            if GetDistanceToTargetFlat() <= (MaxDistance + GetTargetHitboxRadius() + GetPlayerHitboxRadius()) then
                if IPC.vnavmesh.PathfindInProgress() or IPC.vnavmesh.IsRunning() then
                    Engines.Run("/vnav stop")
                    yieldd\("/wait 5.002") -- wait 5s before inching any closer
                elseif (GetDistanceToTargetFlat() > (1 + GetTargetHitboxRadius() + GetPlayerHitboxRadius())) and not Svc.Condition[CharacterCondition.casting] then -- never move into hitbox
                    Engines.Run("/vnav movetarget")
                    yieldd\("/wait 1") -- inch closer by 1s
                end
            elseif not (IPC.vnavmesh.PathfindInProgress() or IPC.vnavmesh.IsRunning()) then
                yieldd\("/wait 5.003") -- give 5s for enemy AoE casts to go off before attempting to move closer
            ...
            AttemptToTargetClosestFateEnemy()
            yieldd\("/wait 1") -- wait in case target doesn't stick
...
function ExecuteBicolorExchange()
    ...
    if BicolorGemCount >= 1400 then
        ...
        if SelectedBicolorExchangeData.miniAethernet ~= nil and
            ...
            yieldd\("/wait 1") -- give it a moment to register
            return
...
ChocoboStance = Config.Get("Chocobo Haltung") -- Options: Follow, Free, Defender, Healer, Attacker, None. Do not summon if None.
ShouldSummonChocobo = ChocoboStance == "Mitkommen" 
    or ChocoboStance == "Freie Haltung"
    or ChocoboStance == "Verteidigen"
    or ChocoboStance == "Heilen"
    or ChocoboStance == "Angreifen"

ShouldAutoBuyGysahlGreens = Config.Get("Gizar-Blatt kaufen?")    --
...
CompletionToIgnoreFate = Config.Get("Ignoriere FATE wenn Fortschritt über (%)")
MinTimeLeftToIgnoreFate = Config.Get("Ignoriere FATE wenn Restzeit wengier als (minuten)")*60
CompletionToJoinBossFate = Config.Get("Ignoriere Boss-FATE bis Fortschritt mindestens (%)")
CompletionToJoinSpecialBossFates = Config.Get("Ignoriere Special-FATE bis Fortschritt mindestens (%)")
...
JoinCollectionsFates = Config.Get("Sammel-FATEs machen?")
BonusFatesOnly = Config.Get("Nur Bonus-FATEs machen?")         --If true, will only do bonus fates and ignore everything else

MeleeDist = Config.Get("Maximale Nahkämpfer-Reichweite")
RangedDist = Config.Get("Maximale Fernkämpfer-Reichweite")
...
EnableChangeInstance = Config.Get("Instanz Wechseln, wenn keine FATESs?")
WaitIfBonusBuff = true          --Don't change instances if you have the Twist of Fate bonus buff
NumberOfInstances = 2
ShouldExchangeBicolorGemstones = Config.Get("Zweifarbige Edelsteine eintauschen?")
ItemToPurchase = Config.Get("Zweifarbige Edelsteine eintauschen gegen")
ShouldExchangeBicolorGemstones = true
if ItemToPurchase == "" or ItemToPurchase == nil then
    ShouldExchangeBicolorGemstones = false
end
SelfRepair = Config.Get("Selbst reparieren?")
...
Retainers = Config.Get("Für Gehilfen pausieren?")
ShouldGrandCompanyTurnIn = Config.Get("Überschüssige Ausrüstung bei GC eintauschen?")         --should it do Turn ins at the GC (requires Deliveroo)
    InventorySlotsLeft              = 5             --how much inventory space before turning in

ReturnOnDeath = Config.Get("Nach Tod zurückkehren?")
